...
@RequestMapping("forms-learns")
@RestController
@AllArgsConstructor
@Tag(name = "Forms Learns Api", description = "Api orientada a brindar informacion las formas de aprendizaje")
@CrossOrigin(origins = "*", methods = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE, RequestMethod.PATCH})
public class FormsLearnsController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "#{" + I18nCodes.API_RESPONSE_CREATED + "}", content = @Content(schema = @Schema(implementation = FormLearnDto.class))), @ApiResponse(responseCode = "404", description = "#{" + I18nCodes.API_RESPONSE_NOT_FOUND + "}"), @ApiResponse(responseCode = "500", description = "#{" + I18nCodes.API_RESPONSE_INTERNAL_SERVER_ERROR + "}")})
    ...
    public ResponseEntity<?> findAll(@RequestParam(name = "page", required = false) Integer page, @RequestParam(name = "size", required = false) Integer size, @RequestParam(name = "sort", required = false) Sort sort) {
